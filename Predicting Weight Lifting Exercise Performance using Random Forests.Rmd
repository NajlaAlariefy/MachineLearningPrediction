...
####Splitting the Data
The value of **0.75** is chosen to split the data into training and testing.
```{r, cache=TRUE}
set.seed(224)
...
Training set has `r trn[1]` observations and the testing set has `r tst[1]` observations.
...
p1 <- predict(model1, test[,-49])
p2 <- predict(model2, test[,-49])
```

###Results 
####Evaluation
The accuracy of the model is a rate higher than **0.98**, and that indicates a low error rate and a very accurate classifier.
...
####Out of Sample Error Measurement: RMSE
Out of sample (or commonly refered to as Out Of Bag (OOB)) error measurement used here is **Root Mean Square Error**. It is calculated by summing the distancing between the predicted values and the truth, squaring them, dividing by the number of observations then taking the square root for readibility.
